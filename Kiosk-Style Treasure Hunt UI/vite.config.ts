...
// Port defaults per room - mirrors game_settings.json so each kiosk
// gets the right port even without a .env.local file.
const ROOM_DEFAULT_PORTS: Record<string, number> = {
  YOGA_ROOM:      5173,
  CTLC_LAB:       5174,
  MUSIC_ROOM:     5175,
  H2_LOUNGE:      5176,
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      // Proxy /api requests to the unified Python backend so the frontend
      // never makes cross-origin requests in development.
      proxy: {
        '/api': {
          target: apiBackend,
          changeOrigin: true,
